...
class SawyerMoveItBridge:
    def __init__(self):
        rospy.loginfo("Starting Sawyer MoveIt unified bridge...")
        ...
        # Bridge /robot/joint_states -> /joint_states
        ...
        # Publisher to Sawyer command topic
        ...
        # Action server expected by MoveIt
        ...
        rospy.loginfo("Unified bridge ready.")
        rospy.loginfo("Relaying: /robot/joint_states -> /joint_states")
        rospy.loginfo("Action server: /robot/limb/right/follow_joint_trajectory")
        rospy.loginfo("Publishing commands to: /robot/limb/right/joint_command")
    ...
    def execute_cb(self, goal):
        ...
        # Publish first point briefly
        for _ in range(5):
            self.publish_position(traj.joint_names, previous_positions)
            rate.sleep()
        ...
        while not rospy.is_shutdown() and (rospy.Time.now() - hold_start).to_sec() < 1.0:
...
if __name__ == "__main__":
    rospy.init_node("sawyer_moveit_bridge")
    SawyerMoveItBridge()
    rospy.spin()
